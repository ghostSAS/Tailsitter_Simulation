...
Q = omega(2);
R = omega(3);
quat(2:4) = x(4:6); % quaternion
quat(1) = sqrt(1 - quat(2)^2 - quat(3)^4 - quat(4)^2);
V = x(7:9); % translational velocities
...
ram_drag(1) = -1/abs(V(1))*0.5*rho*V(1)^3*S_w*0.1;
ram_drag(2) = -1/abs(V(2))*0.5*rho*V(2)^3*S_w*0.1;
ram_drag(3) = -1/abs(V(3))*0.5*rho*V(3)^3*S_w*1.3; % drag from moving in the body z direction [N]
...
F_G = T_eb*[-m*g; 0; 0]; % force of gravity [N]
...
quat_dot = 0.5*omega*quat(1) - 0.5*cross_omega*quat(2:4)';
...
A_lin = double(subs(A_j,x,xeq));
B_lin = double(subs(B_j,u,ueq));
C_lin = double(subs(C_j,x,xeq));
D_lin = double(subs(D_j,u,ueq));

lin_sys = ss(A_lin,B_lin,C_lin,D_lin, 'statename', ...
