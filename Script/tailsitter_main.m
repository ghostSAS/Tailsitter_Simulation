...
cd(folder)
addpath(genpath('helper_fcn'), genpath('Stored_data'))
clear
close all

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
show_3d_animation = false;
show_position_animaition = true;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

q0 = eul2quat([0 pi/2 0])';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% x \in R^13 in the order of [P Q R quat(Psi theta Phi) Vx Vy Vz X Y Z]
% P Q R quat(Psi theta Phi) in the order of [Roll Pitch Yaw]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pos0 = [0; 0; -5];
v0 = [0; 0; 0];
omega0 = [0; 0; 0];
opts = optimoptions(@fsolve,'Algorithm', 'levenberg-marquardt');
xeq = fsolve(@nonlinear_dynamics_eq, [omega0; q0; v0; pos0; 0; 0; 240; 240],opts);

%INPUT REFERENCE ATTITUDE HERE FOR THE DISTURBANCE BELOW
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% default order is 'ZYX' --> [yaw pitch roll]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
flap_max = 20;
flap_rate = 400;
perturbation = zeros(length(x),length(t));
perturbation(2,400:405) = -0 + zeros(1,6);
...
%     u_adap(:,i+1) = [flap_cmd_adap; motor_cmd_adap];
    ...
    x_ode(:,i+1) = ode_out(end,:)' + perturbation(:,i);
...
tout = t';
save('Stored_data/test.mat', 'yout', 'stick', 'tout');

if show_3d_animation
    run_animation_tailsitter;
end

if show_position_animaition
    position_animation;
end