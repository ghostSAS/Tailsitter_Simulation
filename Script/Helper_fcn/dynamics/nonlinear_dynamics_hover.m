function xdot = nonlinear_dynamics_hover(t,x, input, t_vec)
...
u = interp1(t_vec, input, t); % interpolate input signal based on current time t
...
L_P = CP*rho*D^5/(2*pi)*(n_right^2 - n_left^2); % will replace with propeller roll equation
...
F_A = [0; 0; -(lift_left + lift_right)]; % aerodynamic force (lift) [N]

F_P = [T_left + T_right; 0; 0]; % propulsive force [N]

F_G = T_eb*[0; 0; mg]; % force of gravity [N]
